...
function Events() {
  ...
  async function submitEvent() {
    ...
    await supabase.from("events").insert([
      { title: title, datetime: datetime, location: location, category: category, rsvp: "Select your RSVP" },
    ]);
    setTitle("");
    setDatetime("");
    setLocation("");
  ...
  return (
    <div className="phone">
      ...
      <h3 style={{ fontSize: 15, marginBottom: 8 }}>Next Up</h3>
      {upcoming.length === 0 ? (
        <p className="sub">No upcoming events yet, add one below.</p>
      ) : (
